...
// ไลบรารีจาก CDN ที่แอปต้องใช้ตอนออฟไลน์ (Tailwind, Chart.js, xlsx, ฟอนต์)
// แคชแบบ "best effort" ตั้งแต่ตอน install เลย ไม่ต้องรอให้ผู้ใช้เปิดหน้าเว็บสำเร็จก่อน
// (ถ้าเปิดแอปครั้งแรกสุดแล้วเน็ตสะดุด ไฟล์พวกนี้จะไม่เคยถูกแคชเลยถ้าไม่ทำจุดนี้)
const CDN_ASSETS = [
...
self.addEventListener('install', (event) => {
  event.waitUntil((async () => {
    ...
    await cache.addAll(APP_SHELL); // ไฟล์ในเครื่องต้องสำเร็จทุกไฟล์ ไม่งั้น install fail (ตั้งใจ)

    // ไฟล์ CDN: พยายามแคชแยกทีละไฟล์ ถ้าไฟล์ไหนพลาด (เช่น เน็ตหลุดตอนนั้น) ให้ข้ามไป
    // ไม่ให้ทั้ง install ล้มเหลว เดี๋ยว fetch handler ด้านล่างจะไปแคชให้เองตอนใช้งานจริงแทน
    await Promise.all(CDN_ASSETS.map(async (url) => {
      try {
        await cache.add(url);
      } catch (err) {
        console.warn('[SW] precache CDN failed (จะลองแคชใหม่ตอนใช้งานจริงแทน):', url, err);
      }
    }));
